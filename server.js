...
const DEV_JWT_SECRET = "project-galaxy-dev-secret";
const LAUNCHER_VERSION = "1.0.0-local";
const HIDDEN_LEADERBOARD_IDS = new Set(["local-dev-account"]);

const SHOP_ITEMS = [];

const NEWS_POSTS = [
  {
    id: "local-backend-ready",
    title: "Local Backend Online",
    body: "Project Galaxy is wired to your local backend on port 3000.",
  },
  {
    id: "build-1440",
    title: "Build 14.40",
    body: "Backend routes for version check, lightswitch, cloudstorage, profiles, shop, news, and launcher status are ready.",
  },
];
...
async function getLeaderboard() {
  if (firebaseEnabled && firestore) {
    ...
    return snapshot.docs
      .map((doc) => doc.data())
      .filter((entry) => !HIDDEN_LEADERBOARD_IDS.has(entry.id));
  }

  return Object.values(loadDB())
    .map(defaultLeaderboardEntry)
    .filter((entry) => !HIDDEN_LEADERBOARD_IDS.has(entry.id));
}
...
// =====================
// LAUNCHER STATUS, NEWS, SHOP, AND TEST PAGE
// =====================
function launcherStatus(req) {
  const discordReady = Boolean(CLIENT_ID && CLIENT_SECRET && JWT_SECRET);
  return {
    ok: true,
    name: "Project Galaxy",
    ...
    fortnite: {
      build: FORTNITE_BUILD,
      routesReady: true,
      versioncheck: "/fortnite/api/v2/versioncheck/Windows",
      note: "Backend routes are ready. The game client still needs to be directed to this backend before it can use them.",
    },
    presence: { online: getOnlineCount() },
    routes: {
      health: "/health",
...
app.get(["/status", "/launcher/status"], (req, res) => {
  res.json(launcherStatus(req));
});
...
<head>
  ...
  <title>Project Galaxy Backend Test</title>
  <style>
    body { margin: 0; font-family: Arial, sans-serif; background: #101521; color: #f4f7ff; }
    ...
    .card { border: 1px solid #283347; background: #171f2e; border-radius: 8px; padding: 14px; }
    .ok { color: #72f0a8; }
    .warn { color: #ffd26f; }
    pre { white-space: pre-wrap; background: #0a0f18; border: 1px solid #263247; border-radius: 8px; padding: 12px; overflow: auto; }
    button { border: 1px solid #6f85ff; background: #27336a; color: #fff; border-radius: 8px; padding: 9px 12px; cursor: pointer; }
  </style>
</head>
<body>
  <main>
    <h1>Project Galaxy Backend Test</h1>
    <div class="muted">Backend URL: ${baseUrl}</div>
    <div class="grid">
      <div class="card"><strong>Backend</strong><div id="backend" class="warn">Checking...</div></div>
      <div class="card"><strong>Auth</strong><div id="auth" class="warn">Checking...</div></div>
      <div class="card"><strong>Fortnite Routes</strong><div id="fortnite" class="warn">Checking...</div></div>
      <div class="card"><strong>Shop</strong><div id="shop" class="warn">Checking...</div></div>
    </div>
    <button id="run">Run Test Again</button>
  ...
  <script>
    const endpoints = [
      ...
      "/news",
      "/leaderboard",
      "/fortnite/api/v2/versioncheck/Windows?cl=14550713"
    ];
    async function getJson(path) {
      const res = await fetch(path);
      return { path, ok: res.ok, status: res.status, data: await res.json() };
    }
    async function run() {
      ...
      for (const endpoint of endpoints) {
        ...
        catch (err) { results.push({ path: endpoint, ok: false, error: err.message }); }
      }
      const status = results.find((item) => item.path === "/launcher/status")?.data || {};
      const health = results.find((item) => item.path === "/health")?.data || {};
      const version = results.find((item) => item.path.startsWith("/fortnite/api")) || {};
      const shop = results.find((item) => item.path === "/shop")?.data || {};
      document.getElementById("backend").textContent = health.ok ? "Online" : "Problem";
      document.getElementById("backend").className = health.ok ? "ok" : "warn";
      document.getElementById("auth").textContent = status.auth?.discordReady ? "Discord ready" : "Local dev login ready";
      document.getElementById("auth").className = "ok";
      document.getElementById("fortnite").textContent = version.ok ? "Version check returns " + version.data.type : "Problem";
      document.getElementById("fortnite").className = version.ok ? "ok" : "warn";
      document.getElementById("shop").textContent = Array.isArray(shop.items) ? shop.items.length + " items loaded" : "Problem";
      document.getElementById("shop").className = Array.isArray(shop.items) ? "ok" : "warn";
...
app.get("/verify", (req, res) => {
  try {
    const data = jwt.verify(req.query.token, jwtSecret());
